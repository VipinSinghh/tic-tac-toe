...
export function Game() {
  ...
  const checkGameState = (matrix: IPlayMatrix) => {
    for (let i = 0; i < matrix.length; i++) {
      ...
      if (row.every((value:any) => value && value === playerSymbol)) {
        return [true, false];
      } else if (row.every((value:any) => value && value !== playerSymbol)) {
        return [false, true];
      }
    }
    ...
    for (let i = 0; i < matrix.length; i++) {
      ...
      if (column.every((value:any) => value && value === playerSymbol)) {
        return [true, false];
      } else if (column.every((value:any) => value && value !== playerSymbol)) {
        return [false, true];
      }
    }
